...
std::vector<int> PermutationEqui(int n_max, int seed) {
    ...
    // Revolvemos el vector
    for (int i = SIZE_ARR_M - 1; i > 0; i--) {
        std::swap(arr[i], arr[g() % (i + 1)]);
    }
...
int main(){
    std::vector<int> arr = PermutationEqui(10, 1234);
    const int SIZE_ARR_M = 1<<28;
    ...
    std::vector<int> arr2 = PermutationEqui(10, 5678);
    assert(arr != arr2);
    std::cout << "Paso\n" << std::endl;
    ...
    std::vector<int> arr3 = PermutationEqui(10, 5678);
    assert(arr2 == arr3);
    std::cout << "Paso\n" << std::endl;
